...
export default function KelolaTiketHilang({ onNav, ticketsHilang = [], setTicketsHilang = () => {}, setSelectedTicket }) {
  const [filter, setFilter] = useState("semua");
  const [loading, setLoading] = useState({});
  const [localTickets, setLocalTickets] = useState(ticketsHilang);
  ...
  useEffect(() => {
    const fetchTickets = async () => {
      try {
        ...
        const hilangTickets = res.data.filter(t => t.tipe === "hilang");
        setLocalTickets(hilangTickets);
        if (setTicketsHilang) setTicketsHilang(hilangTickets);
      } catch (error) {
  ...
  const displayTickets = localTickets && localTickets.length > 0 ? localTickets : ticketsHilang || [];
  const filtered = displayTickets.filter(t => filter === "semua" || t.status === filter);
  ...
  const handleTerima = async (id) => {
    try {
      ...
      alert(error.response?.data?.detail || "Gagal memperbarui tiket");
    } finally {
  ...
  return (
    <AppLayout activePage="admin_tiket_hilang" onNav={onNav} title="Tiket Kehilangan" isAdmin={true}>
      <div style={{ maxWidth: 1280, margin: "0 auto" }}>
        <div style={{ background: "#fef3c7", borderRadius: 12, padding: 16, display: "flex", gap: 12, alignItems: "center", marginBottom: 24, border: "1px solid #fde68a" }}>
          <Icon name="search_off" color="#b45309" size={20} />
          <p style={{ fontSize: 14, color: "#92400e", fontWeight: 500, margin: 0 }}>
            Terdapat <strong>{displayTickets.filter(t => t.status === "menunggu").length}</strong> laporan kehilangan menunggu tindakan Anda.
          </p>
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          {filtered.map((t, i) => (
            <div key={i} style={{ background: "white", borderRadius: 16, padding: 24, boxShadow: "0 1px 3px rgba(0,0,0,0.05)", border: "1px solid #f1f5f9" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                <div style={{ display: "flex", gap: 16, alignItems: "flex-start" }}>
                  <div style={{ width: 64, height: 64, borderRadius: 12, overflow: "hidden", flexShrink: 0, background: "#f1f5f9" }}>
                    {t.img ? <img src={t.img} alt={t.item || t.nama_barang} style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : <Icon name="inventory_2" size={32} color="#94a3b8" style={{ margin: "16px" }} />}
                  </div>
                  <div>
                    ...
                    <h3 style={{ fontSize: 17, fontWeight: 600, marginBottom: 6 }}>{t.nama_barang || t.item}</h3>
                    <div style={{ display: "flex", gap: 16, fontSize: 13, color: "#64748b" }}>
                      <span>📍 {t.lokasi || t.location}</span>
                      <span>👤 {t.nama} ({t.nim})</span>
                      <span>🕐 {t.waktu_kejadian || t.time}</span>
                    </div>
                    <p style={{ fontSize: 13, color: COLORS.onSurfaceVariant, marginTop: 6, maxWidth: 500 }}>{(t.deskripsi || t.desc || "").slice(0, 100)}...</p>
                  </div>
                </div>
                <div style={{ display: "flex", gap: 8, flexShrink: 0, flexDirection: "column" }}>
                  {t.status === "menunggu" && (
                    <div style={{ display: "flex", gap: 8 }}>
                      <button onClick={() => handleTerima(t.id)} disabled={loading[t.id]} style={{ padding: "10px 18px", background: "#dcfce7", color: "#16a34a", borderRadius: 8, fontWeight: 700, fontSize: 13, border: "none", cursor: loading[t.id] ? "not-allowed" : "pointer", opacity: loading[t.id] ? 0.6 : 1 }}>
                        {loading[t.id] ? "..." : "✓ Terima"}
                      </button>
                  ...
                  {(t.status === "diproses") && !t.adminConfirm && (
                    <button onClick={() => handleAdminConfirm(t.id)} disabled={loading[t.id]} style={{ padding: "10px 18px", background: "#fffbeb", color: "#b45309", borderRadius: 8, fontWeight: 700, fontSize: 13, border: "1px solid #fde68a", cursor: loading[t.id] ? "not-allowed" : "pointer", opacity: loading[t.id] ? 0.6 : 1 }}>
                      {loading[t.id] ? "..." : "🤝 Konfirmasi Ditemukan"}
                    </button>
                  )}
                  {t.status === "selesai" && (
